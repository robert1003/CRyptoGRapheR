struct Bigint{
  static const int LEN=60,BIGMOD=10000;
  int s,vl,v[LEN]; //  vector<int> v;
  Bigint():s(1){ vl=0; }
  Bigint(long long a){
    s=1; vl=0; if(a<0){ s=-1; a=-a; }
    while(a){ push_back(a%BIGMOD); a /= BIGMOD; }
  }
  Bigint(string str){
    s=1;vl=0; int stPos=0,num=0;
  ...
  friend std::ostream& operator<<(std::ostream& out,const Bigint &a){
    ...
    for(int i=a.len()-2;i>=0;i--){
      char str[10]; snprintf(str,5,"%.4d",a.v[i]);
      out<<str;
    }
    return out;
  }
  ...
  Bigint operator+(const Bigint &b) const{
    ...
    Bigint r; int nl=max(len(),b.len()); r.resize(nl+1);
    // directly add TODO (r = *this + b) // r[i]<BIGMOD
    r.n(); return r;
  }
  Bigint operator-(const Bigint &b) const{
    if(s==-1) return -(-(*this)-(-b));
    ...
    // directly sub TODO (r = *this - b) // r[i]<BIGMOD
    r.n(); return r;
  }
  Bigint operator*(const Bigint &b){
    Bigint r; r.resize(len()+b.len()+1); r.s=s*b.s;
    // directly mul TODO (r = *this * b) // r[i]<BIGMOD
    r.n(); return r;
  }
  Bigint operator/(const Bigint &b){
    Bigint r; r.resize(max(1,len()-b.len()+1));
    int oriS=s; Bigint b2=b; s=b2.s=r.s=1; // b2=abs(b)
    for(int i=r.len()-1;i>=0;i--){
      int d=0,u=BIGMOD-1;
      while(d<u){
        int m=(d+u+1)>>1; r.v[i]=m;
        if((r*b2)>(*this)) u=m-1; else d=m;
      }
      r.v[i]=d;
    }
    s=oriS; r.s=s*b.s; r.n(); return r;
  }
  Bigint operator%(const Bigint &b){
