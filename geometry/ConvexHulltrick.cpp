...
struct Conv{
  ...
  Conv(vector<Pt> _a):a(_a){
    ...
    upper.push_back(a[0]);
  } // sign: modify when changing to double
  ...
  int bi_search(Pt u,Pt v,int l,int r){
    ...
    while(l+1<r){
      int mid=(l+r)/2,smid=sign(det(v-u,a[mid%n]-u));
      if(smid==sl) l=mid; else r=mid;
    }
    return l%n;
  }
  ...
  bool get_intersection(Pt u,Pt v,int &i0,int &i1){ 
   ...
   if(sign(det(v-u,a[p0]-u))*sign(det(v-u,a[p1]-u))<0){
     if(p0>p1) swap(p0,p1);
     i0=bi_search(u,v,p0,p1); i1=bi_search(u,v,p1,p0+n);
     return 1;
   }
   return 0;
  }
