// Pt:{x,y,z}
#define N 202020
int n,nouter; Pt pt[N],outer[4],res; double radius,tmp;
...
void ball(){
  ...
  switch(nouter){
    ...
    case 4:
      ...
      for(i=0;i<3;++i) 
        for(j=0;j<3;++j) m[i][j]=(q[i]*q[j])*2;
      d=det(m); if(fabs(d)<eps) return;
      for(j=0;j<3;++j){
        for(i=0;i<3;++i) m[i][j]=sol[i];
        L[j]=det(m)/d;
        for(i=0;i<3;++i) m[i][j]=(q[i]*q[j])*2;
      } 
      res=outer[0]; for(i=0;i<3;++i) res=res+q[i]*L[i];
