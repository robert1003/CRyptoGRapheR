...
const int V=66,T=10; const ll INF=1023456789;
struct SteinerTree{
  ...
  void shortest_path(){
  	for(int i=0;i<n;i++) for(int j=0;j<n;j++)
        if(i!=j&&dst[i][j]!=INF) dst[i][j]+=w[i];
    for(int k=0;k<n;k++) for(int i=0;i<n;i++)
        for(int j=0;j<n;j++)
          dst[i][j]=min(dst[i][j],dst[i][k]+dst[k][j]);
    for(int i=0;i<n;i++) for(int j=0;j<n;j++)
        if(dst[i][j]!=INF) dst[i][j]+=w[j];
  }
  int solve(const vector<int>& ter){
    ...
    for(int msk=1;msk<(1<<t);msk ++){
      if(msk==(msk&(-msk))){
        int who=__lg(msk);
        for(int i=0;i<n;i++)dp[msk][i]=dst[ter[who]][i];
        continue;
      }
      for(int i=0;i<n;i++)
        for(int submsk=(msk-1)&msk;submsk;submsk=(submsk-1)&msk)
          dp[msk][i]=min(dp[msk][i],dp[submsk][i]+dp[msk^submsk][i]-w[i]);
      for(int i=0;i<n;i++){
        tdst[i]=INF;
        for(int j=0;j<n;j++) tdst[i]=
          min(tdst[i],dp[msk][j]+dst[j][i]-w[j]);
      }
      for(int i=0;i<n;i++) dp[msk][i]=tdst[i];
    }
    int ans=INF;
    for(int i=0;i<n;i++) ans=min(ans,dp[(1<<t)-1][i]);
    return ans;
  }
} solver;
