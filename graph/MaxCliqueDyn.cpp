...
struct MaxClique{ // Maximum Clique
  bitset<N> a[N],cs[N]; int ans,sol[N],q,cur[N],d[N],n;
  void init(int _n){
  ...
  void csort(vector<int> &r,vector<int> &c){
    ...
    for(int i=0;i<m;i++){
      ...
      cs[k][p]=1; if(k<km) r[t++]=p;
    }
    c.resize(m); if(t) c[t-1]=0;
    for(int k=km;k<=mx;k++){
  ...
  void dfs(vector<int> &r,vector<int> &c,int l,bitset<N> mask){
    while(!r.empty()){
      ...
      else if(q>ans){ ans=q; copy(cur,cur+q,sol); }
      c.pop_back(); q--;
    }
  }                // vertex mask
  int solve(bitset<N> mask=bitset<N>(string(N,'1'))){
    ...
    return ans; // vertices set: sol[0 ~ ans-1]
  }
}graph;
