struct WeightGraph {
  ...
  edge g[N*2][N*2]; vector<int> flo[N*2]; queue<int> q;
  ...
  void set_match(int u,int v){
    match[u]=g[u][v].v; if(u<=n) return; edge e=g[u][v];
    int xr=flo_from[u][e.u],pr=get_pr(u,xr);
    for(int i=0;i<pr;++i) 
      set_match(flo[u][i],flo[u][i^1]);
    set_match(xr,v); rotate(flo[u].begin(),flo[u].begin()+pr,flo[u].end());
  }
    ...
    for(size_t i=0;i<flo[b].size();++i){
      ...
      for(int x=1;x<=n_x;++x)
        if(g[b][x].w==0||e_delta(g[xs][x])<e_delta(
            g[b][x])) g[b][x]=g[xs][x],g[x][b]=g[x][xs];
      for(int x=1;x<=n;++x)
  ...
  bool matching(){
    ...
    for(;;){ while(q.size()){
      ...
      for(int u=1;u<=n;++u){ if(S[st[u]]==0){
          if(lab[u]<=d) return 0; lab[u]-=d;
      ...
      for(int b=n+1;b<=n_x;++b) if(st[b]==b&&S[b]==1&&
